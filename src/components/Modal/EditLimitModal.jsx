...
import { handleUpdateDownloadLimit } from "../../utils/data";
import { updateAdLimit } from "../../Redux/AdsSlice/AdsSlice";
...
const EditLimitModal = () => {
  ...
  const handleUpdateLimit = async (event) => {
    ...
    setLoading(true);
    if (!editLimit)
      return handleAsyncError(dispatch, "No Id Found.Try Again..");
    if (editLimit?.limit == limit || limit == " ")
      return handleAsyncError(dispatch, "Limit should not be same or empty.");
    const response = await handleUpdateDownloadLimit(
      editLimit?.id,
      limit.toString(),
      editLimit?.totalDownload
    );
    if (response.type == "success") {
      handleAsyncError(dispatch, response.message, response.type);
      // updating the state
      ...
      handleAsyncError(
        dispatch,
        "Something Went Wrong While updating limit...."
      );

...
import { handleUpdateUserProfile } from "../../utils/data";
import Spinner from "../spinner/Spinner";
import { handleAsyncError } from "../../utils/Helper/handleAsyncError";
import { useDispatch, useSelector } from "react-redux";
import handleCheckUser from "../../utils/data/handleCheckUser";
import { handleProfileUpdate } from "../../Redux/currentUserSlice/CurrentUserSlice";
...
const EditProfile = ({ isEditMode, setEditModeChanger }) => {
  ...
  const handleUpdateProfile = async (e) => {
    setLoading(true);
    e.preventDefault();
    const updatedUserName = currentUser?.userName != userName ? userName : "";
    ...
    if (response.type == "success") {
      const updateUser = await handleCheckUser(currentUser?.phone);
      // updating the localstate of user
      if (updateUser.type == "success") {
        dispatch(
          handleProfileUpdate({
            userName: updateUser?.data?.UserName,
            userProfileImage: updateUser?.data?.UserProfileImageUri,
          })
        );
      }
      handleAsyncError(dispatch, response.message, response.type);
      setEditModeChanger(false);
    } else {
